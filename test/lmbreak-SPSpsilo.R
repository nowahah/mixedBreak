# remotes::install_github("bozenne/lmbreak")
...
SPSpsilo <- basel %>% filter(Study=="LPM")
summary(SPSpsilo)
...
e.XPall1010 <- mlmbreak(
  score ~ 0 + bp(time, c("1010")), #, "101", "11")), 
  # one pattern can't rule them all
  cluster = "ID",
  data = SPSpsilo, trace = FALSE
)
# plot(e.XPall101)
...
mean(breaks$bp.2 - breaks$bp.1, na.rm = TRUE)
sd(breaks$bp.2 - breaks$bp.1, na.rm = TRUE)
range(breaks$bp.2 - breaks$bp.1, na.rm = TRUE)
...
coeffs <- coef(e.XPall1010, type=c("slope")) %>%
  group_by(ID) %>% 
...
round(10*c(mean(slopes$beta.3, na.rm = TRUE), 
           sd(slopes$beta.3, na.rm = TRUE), 
           range(slopes$beta.3, na.rm = TRUE)), 2)
