...
o <- segreg(score ~  0 + seg(time, npsi=n.breaks, est=c(1,0,1)), data=SDIpsilo) # forcing plateau
summary(o)
...
p <- ggplot(SDIpsilo.ind, aes(x = time, y = score, colour = type, shape = type)) +
  ...
  # labels
  labs(
    title = paste("Subjective Drug Intensity over time for patient:", patient),
...
p +
  geom_segment(aes(
    x = bp.ci[1, 1], y = predict(o, data.frame(time = bp.ci[1, 1])),
    xend = bp.ci[2, 1], yend = predict(o, data.frame(time = bp.ci[2, 1]))
  ))

# segmented plot
plot.segmented(o,
  conf.level = .95, res = T, ylim = c(0, 12),
