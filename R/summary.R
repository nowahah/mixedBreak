...
##' @export
summary.break.lme1 <- function(z, default = FALSE){
  if(default) {
  ...
  resd <- resid(x, type = "pearson") #... Muggeo does lme.fit.noG
  if (length(resd) > 5) {
  ...
  row.names(mod.coef)[var.G] <- paste0("break.", 1:length(which(var.G)))
  print(mod.coef[,-3]) # AAAH TODO - psi is still on logit scale
  
  # TODO - check validity since Muggeo ecluded breakpoint estimate from the fit here
