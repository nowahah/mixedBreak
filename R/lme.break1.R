    ...
    psi.history = TRUE, model = TRUE, x = FALSE, y = FALSE)
{
  ...
  while((logLik.diff > tol.logLik | logLik.diff < 0) & it < it.max){
    ...
    XX$U <- pmax(0, XX[[vars$segmented]] - psi.history[it,1,XX[[vars$group]]])
    XX$V <- -1 * (XX[[vars$segmented]] > psi.history[it,1,XX[[vars$group]]])
    XX$D <- ((exp(eta.i) * (a2 - a1)) / (1 + exp(eta.i))^2)[XX[[vars$group]]]
    XX$G <- delta.i[XX[[vars$group]]] * XX$V * XX$D
    XX$O <- -eta.i[XX[[vars$group]]] * XX$G
    ...
    logLik.diff <- logLik(mod.work) - logLik(mod.work.prev)
    ...
    eta.i <- coef(mod.work)[[vars$group]]$G
    psi.history[it+1,1,] <- expit(eta.i)
    delta.i <- coef(mod.work)[[vars$group]]$U
